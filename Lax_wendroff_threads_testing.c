...
static double u_n[X+1][Y+1];
static double u[X+1][Y+1];
static double u_p[X+1][Y+1];
static double r_mat[X+1][Y+1];
static double r_mat_n[X+1][Y+1];
static double s_mat[X+1][Y+1];
static double s_mat_n[X+1][Y+1];
static double s_mat_p[X+1][Y+1];
static double l_mat[X+1][Y+1];
static double l_mat_n[X+1][Y+1];
...
void main()
{
	...
	for (t = 0; t <= T; t++) {
		...
		t3 = clock();
   		t_thread_total +=(thread_end-thread_start) / CLOCKS_PER_SEC;
	...
	printf("%f s  and   %f s and %f s and %f s\n", t_thread_total, (t3 - t2) / CLOCKS_PER_SEC, (t3 - t0) / CLOCKS_PER_SEC, (t3 - t1) / CLOCKS_PER_SEC);
//	for (i=0,i<T+1,i++){
//			for (j=0,j<N,j++){
//					t_store=t_fun[i][j];
...
//	int            n =  (int) (intptr_t) arg;  //Nth thread
    int n = 0;
	double t_fun_start,t_fun_end,t_fun_l;
	...
	t_fun_end=clock();
	t_fun_l=(t_fun_end-t_fun_start)/CLOCKS_PER_SEC;
	printf("t_fun[][]= %f, process_count = %d\n",t_fun_l,process_count);
